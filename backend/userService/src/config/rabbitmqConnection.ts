...
let connection: amqplib.ChannelModel | null = null;  // must be only one connection per service
let publisherChannel: amqplib.Channel | null = null; // more than one channel can be create, here we need only one
...
export const sendOtpToQueue = async (otpPayload: unknown, options: {retries?: number, delayMs?: number}): Promise<boolean> => {
